...
function updatePluginStatus(status, type) {
    ...
    if (type === 'checking') {
        sectionElement.classList.add('status-checking');
    } else if (type === 'success') {
        sectionElement.classList.add('status-success');
    } else if (type === 'error') {
        sectionElement.classList.add('status-error');
    }
}
...
function startWatchingLocation() {
    try {
        ...
        const onSuccess = function (position) {
            ...
            // แยกแต่ละข้อมูลให้อยู่คนละบรรทัด
            const locationInfo = `
                Latitude: ${latitude.toFixed(6)}<br>
                Longitude: ${longitude.toFixed(6)}<br>
                Accuracy: ${accuracy} meters<br>
                Altitude: ${altitude ? altitude + ' meters' : 'N/A'}<br>
                Heading: ${heading ? heading + '°' : 'N/A'}<br>
        ...
        const onError = function (error) {
            ...
            updateLocationStatus('Error getting location: ' + error.message, 'error');
        };
...
function updateLocationStatus(status, type) {
    ...
    locationElement.innerHTML = status; // ใช้ innerHTML เพื่อรองรับ <br>
